...
S'ha detectat que els valors màxims de PM10 reals, eliminem els outliers d'aquestes dades errónies aportades per les estacions de mesura:

```{r, warning=FALSE, message=FALSE}

# Calculem el rang interquartílic (IQR) per a PM10
pm10_limits <- contaminants_long %>%
  ...
  mutate(
    ...
    límit_inferior = q1 - 1.5 * rang_interquartilic,
    límit_superior = q3 + 1.5 * rang_interquartilic
  )

# Extraiem els límits
límit_inf <- pm10_limits$límit_inferior
límit_sup <- pm10_limits$límit_superior

# Eliminem només els outliers de PM10, la resta de contaminants es mantenen
contaminants_long <- contaminants_long %>%
  filter(
    contaminant != "PM10" |
      (valor >= límit_inf & valor <= límit_sup)
  )

```

Tornem a calcular les estadístiques bàsiques:
...
-   **PM10:** Amb 644.985 mesures, la mediana és de 22 µg/m³ i la mitjana de 24,68 µg/m³. Tot i que la majoria de les concentracions són moderades, s’observa un valor màxim extrem de 65 µg/m³, indicant episodis puntuals amb alta concentració, però no és significatiu en comparació a la resta de contaminants.
...
La mediana (línia vermella) ajuda a veure on es localitza la major part de les observacions, i la cua a la dreta en NO2, PM10 i PM2.5 ens indica que, tot i que la majoria de registres no són excessivament alts, hi ha episodis concrets on les concentracions pugen significativament.
...
-   **PM10:** Les estacions de trànsit també solen presentar valors més alts que les urbanes, reflectint la contribució del trànsit rodat (desgast de pneumàtics, frenades, pols en suspensió) a l’augment de partícules de mida superior a 10 µm. En dècades recents es pot observar una lleugera millora, però encara es mantenen pics puntuals.

-   **PM2.5:** Tot i que hi ha menys dades en algunes franges de temps, en general també es perceben concentracions més elevades a la zona de trànsit que a la urbana. Les partícules fines (inferiors a 2,5 µm) tenen un impacte significatiu en la salut, i l’activitat del trànsit n’és una font rellevant, juntament amb altres processos de combustió.
