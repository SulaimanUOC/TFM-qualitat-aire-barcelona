...
-   **Comparativa per tipus d’estació** (trànsit vs. urbana de fons).
...
df_meteo <- read_csv("../data/processed/meteocat/meteocat_2021_2025_bcn_processed.csv")
df_contaminants <- read_csv("../data/processed/contaminants/contaminants_bcn_filtrat.csv")
```

Convertim les dades horàries en un format "long" per facilitar l’anàlisi.
...
En aquesta visualització, s’hi comparen les concentracions mitjanes horàries de quatre contaminants (**NO2**, **O3**, **PM10** i **PM2.5**) en funció del **tipus d’estació**—concretament, estacions de fons urbà (**background**) i estacions situades en zones de trànsit (**traffic**)—al llarg del temps. A grans trets, s’hi observen els patrons següents:

-   **NO2:** Les estacions de trànsit (línia en color blau/verd) mostren clarament valors més elevats que les de fons urbà (línia en rosat), ja que el diòxid de nitrogen es genera majoritàriament per combustió de vehicles. Tot i que en èpoques recents sembla apreciar-se un lleuger descens, segueix existint una diferència notable entre ambdós tipus d’estació.

-   **O3:** De manera contrària, en molts intervals de temps es detecten concentracions més altes a les estacions de fons urbà que a les de trànsit. Això succeeix perquè, al costat de vies amb molt trànsit, els òxids de nitrogen (NO, NO2) poden “titrar” (destruir) part de l’ozó. Per tant, en zones més allunyades de les fonts de NO, els nivells d’O3 tendeixen a ser superiors.
...
En definitiva, la comparativa evidencia que les estacions de **trànsit** registren concentracions clarament més elevades de contaminants primaris (NO2, partícules) degut a l’emissió directa dels vehicles, mentre que les estacions de **fons urbà** sovint mostren nivells relativament menors per a aquests contaminants però poden presentar concentracions més altes d’**O3**, a causa dels processos fotoquímics i l’efecte de titració del NO en zones de trànsit. Aquestes diferències remarquen la necessitat de desplegar estratègies ambientals i de mobilitat que tinguin en compte tant la contaminació local (trànsit) com la de caràcter més difús (ozó, partícules fines), per tal de millorar globalment la qualitat de l’aire urbà.
...
1.  **Relació entre contaminants:**
    ...
    -   **NO2 i PM10** també presenten una correlació positiva moderada (entorn de 0,4), cosa coherent amb el trànsit rodat com a font comuna d’aquests contaminants.

    -   **O3** mostra correlacions negatives amb NO2, PM10 i PM2.5 (entorn de -0,3), consistent amb el fenomen de “titració” de l’ozó per l’NO i amb el fet que, en condicions de molt trànsit, tendeix a haver-hi menys O3 a prop de les fonts emissives.
...
3.  **Paper de la precipitació i la humitat:**
    ...
    -   La **precipitació** (PPT) i les variables d’humitat (HRN, HRX) mostren correlacions negatives amb la majoria de contaminants. Això és conseqüència de l’efecte de rentatge (washout) que redueix la concentració de partícules i gasos contaminants en dies de pluja o humitat elevada.

    -   Quan plou o la humitat és alta, els contaminants tendeixen a deposar-se més ràpidament, fet que ajuda a netejar l’aire.
...
1.  **Diferències entre estacions de trànsit i de fons urbà:**

    -   Les estacions ubicades en zones de trànsit intens (com Eixample, Sants, etc.) presenten nivells de **NO2** relativament més elevats. Aquesta dada reflecteix l’origen principalment vehicular d’aquest contaminant.

    -   Les estacions més allunyades del trànsit o situades en zones més altes (com Observatori Fabra) mostren menys acumulació de NO2 i, en canvi, tendeixen a tenir valors més alts d’**O3**, ja que hi ha menys titració de l’ozó per òxids de nitrogen.
...
4.  **Variabilitat entre barris:**
    ...
    -   D’altres, com Palau Reial o Sant Gervasi, tot i que també registren NO2, poden mostrar un perfil mixt (fons urbà / trànsit), reflectint la variabilitat urbana de Barcelona.
...
Les **estacions de trànsit** registren sistemàticament nivells més elevats de contaminants primaris com NO2, PM10 i PM2.5, mentre que les **estacions de fons urbà** mostren valors més alts d’ozó a causa del fenomen de titració i l’absència d’emissions directes de NO. Aquesta divergència posa de manifest la necessitat de tenir en compte la **tipologia d’estació** per a la interpretació adequada dels nivells de contaminació.
...
Les anàlisis per estació mostren una **variabilitat considerable entre barris**, amb concentracions més elevades en zones densament urbanitzades i de trànsit intens com l’Eixample o Sants. En canvi, estacions com l’Observatori Fabra o Sant Gervasi presenten perfils diferenciats, més propis de zones amb menys activitat emissora directa.
